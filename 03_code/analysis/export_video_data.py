#!/usr/bin/env python3
"""
Export all TikTok video data to comprehensive CSV file
Combines data from Apify JSON files and individual metadata files
"""
import os
import json
import csv
...
def calculate_engagement_rate(video_data):
    ...
    views = video_data.get('playCount', 1)  # Avoid division by zero
...
def process_video_data(video_data, search_query):
    ...
    # Extract modality/workout type from text and hashtags
    ...
    return {
        ...
        # Local file info
        'has_local_video': False,  # Will update this later
        'local_video_filename': ''
    }

def main():
    print("🚀 Starting TikTok video data export...")
    
    all_videos = []
    video_ids_processed = set()
    
    # First, get all video files to create a lookup
    video_files = {}
    for filename in os.listdir(VIDEOS_DIR):
        if filename.endswith('.mp4'):
            # Extract video ID from filename (format: query_creator_videoid.mp4)
    ...
    print(f"📊 Processed {len(all_videos)} unique videos")
    
    # Sort by engagement rate
    all_videos.sort(key=lambda x: x['engagement_rate'], reverse=True)
    
    # Write to CSV
    output_file = os.path.join(OUTPUT_DIR, f'tiktok_videos_all_{datetime.now().strftime("%Y%m%d")}.csv')
    ...
    print(f"📈 Top 5 videos by engagement rate:")
